...
export default function JobApplicationUpdateForm(props) {
  const {
    id: idProp,
    jobApplication: jobApplicationModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = jobApplicationRecord
      ? { ...initialValues, ...jobApplicationRecord }
      : initialValues;
    ...
    setPreviousExprience(cleanValues.PreviousExprience);
    setCV(cleanValues.CV);
    setAge(cleanValues.Age);
    setInterviewed(cleanValues.Interviewed);
    setInterviewNotes(cleanValues.InterviewNotes);
    setOffered(cleanValues.Offered);
    setInterviewTime(cleanValues.InterviewTime);
    setInterviewDate(cleanValues.InterviewDate);
    setInterviewSet(cleanValues.InterviewSet);
    setNotInterested(cleanValues.NotInterested);
    setErrors({});
  };
  const [jobApplicationRecord, setJobApplicationRecord] = React.useState(
    jobApplicationModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(JobApplication, idProp)
        : jobApplicationModelProp;
      setJobApplicationRecord(record);
    };
    queryData();
  }, [idProp, jobApplicationModelProp]);
  React.useEffect(resetStateValues, [jobApplicationRecord]);
  const validations = {
    Name: [],
    CoverLetter: [],
    Role: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            JobApplication.copyOf(jobApplicationRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "JobApplicationUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || jobApplicationModelProp)}
